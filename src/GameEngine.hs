            ...
            case rest of
                [] -> 
                    Right $ ofGameState 
                        ...
                        , currentTurn = 0 
                        , privateInfo = []
                        , logs = logs ofGameState ++ ["All players ready. Player 0 starts."]
                        }
                (nextP:_) -> 
                    Right $ ofGameState 
                        { phase = InitialPeekPhase rest
                        , currentTurn = nextP 
                        , privateInfo = []
                        }
...
-- 1. PEEK SELF 
applyPowerupLogic gs PeekSelf t [i] = 
    let me = currentPlayer gs
    in Right (gs, "PeekSelf: " ++ safeGetCard me i)

-- 2. PEEK OPPONENT
applyPowerupLogic gs PeekOpponent t [i] = 
    let opp = (players gs) !! t
    in Right (gs, "PeekOpponent (P" ++ show t ++ "): " ++ safeGetCard opp i)

-- 3. PEEK SO 
applyPowerupLogic gs PeekSO t [myIdx, oppIdx] =
    let me  = currentPlayer gs
        opp = (players gs) !! t
        info = "ME [" ++ show myIdx ++ "]: " ++ safeGetCard me myIdx ++ 
...
applyPowerupLogic gs PeekSwitch t [myIdx, oppIdx] =
    let me  = currentPlayer gs
        opp = (players gs) !! t
        -- Ambil info kartu SEBELUM ditukar untuk ditampilkan
        c1  = safeGetCard me myIdx
        c2  = safeGetCard opp oppIdx
        msg = "SWITCHED! You gave: " ++ c1 ++ " <--> Received: " ++ c2
        
        -- Lakukan pertukaran state
        newState = executeSwap gs (currentTurn gs) myIdx t oppIdx
    in Right (newState, msg)

-- 5. SWITCH 
applyPowerupLogic gs Switch t [myIdx, oppIdx] =
    let newState = executeSwap gs (currentTurn gs) myIdx t oppIdx
    in Right (newState, "Switch Performed successfully.")

-- 6. PEEK DOUBLE 
applyPowerupLogic gs PeekDouble t [m1, m2, o1, o2] =
    let me  = currentPlayer gs
        opp = (players gs) !! t
        infoMe  = "ME: " ++ safeGetCard me m1 ++ " & " ++ safeGetCard me m2
        infoOpp = "OPP (P" ++ show t ++ "): " ++ safeGetCard opp o1 ++ " & " ++ safeGetCard opp o2
    in Right (gs, infoMe ++ " || " ++ infoOpp)

-- Fallback Error
applyPowerupLogic _ p _ args = Left $ "Invalid arguments for " ++ show p ++ ": " ++ show args
...
logicKabul ofGameState callerId = 
    let playerList  = players ofGameState
        ...
        scoreMsg    = "SCORES: " ++ unwords (map (\(id, s) -> "P" ++ show id ++ "=" ++ show s) allScores)

        otherPlayers = filter (\player -> playerId player /= callerId) playerList
        minOther     = minimum (map (handScore.hand) otherPlayers)

        finalGameState = if cScore <= minOther 
                         then applyRankingPoints ofGameState 
                         else applyPenaltyPoints ofGameState callerId
                         
    in Right $ finalGameState 
        { phase = GameOver
        , logs  = logs ofGameState ++ [scoreMsg, "Game Over. " ++ showResults (players finalGameState)] 
        }
...
showResults :: [Player] -> String
showResults ps = "Winner is P" ++ show (playerId (head (sortBy (flip compare `on` matchPoints) ps)))
...
applyRankingPoints ofGameState = 
    let sorted = sortBy (compare `on` (handScore.hand)) (players ofGameState) 
        pointsMap = zip (map playerId sorted) [3, 2, 1, 0]
...
applyPenaltyPoints ofGameState badActorId = 
    ofGameState 
    { 
        players = map (\p -> if playerId p == badActorId 
                             then p { matchPoints = matchPoints p - 5 } 
                             else p) (players ofGameState) 
    }
